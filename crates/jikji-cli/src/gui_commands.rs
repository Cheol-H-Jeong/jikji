...
<body>
  <header class="topbar">
    ...
    <div class="token-wrap"><label class="eyebrow" for="manage-token">Manage token</label><input id="manage-token" type="password" autocomplete="off" spellcheck="false" aria-describedby="token-help" placeholder="Required for changes"></div>
  </header>
  ...
  <section class="stats" aria-label="Index health">
    <div class="stat"><span class="eyebrow">Index health</span><b id="health">Loading status…</b></div><div class="stat"><span class="eyebrow">Indexed files</span><b id="file-count">—</b></div><div class="stat"><span class="eyebrow">Root size</span><b id="root-size">—</b></div><div class="stat"><span class="eyebrow">Last indexed</span><b id="last-indexed">—</b></div>
  </section>
  <section class="deep-status" aria-labelledby="deep-title">
    <div class="pane-head"><h2 id="deep-title">Deep indexing</h2><span class="spacer"></span><span class="eyebrow" id="deep-state">Not run</span></div>
    <p id="deep-copy" class="add-root-copy">Media and archive bodies are opt-in. Configure bounded OCR/ASR and archive processing before starting.</p>
    <div class="controls"><label class="control-field"><input id="media-ocr" type="checkbox"> OCR</label><label class="control-field"><input id="media-asr" type="checkbox"> ASR</label><label class="control-field">Archive entries <input id="archive-entries" type="number" min="1" step="1" value="1000"></label><label class="control-field">Entry bytes <input id="archive-entry-bytes" type="number" min="1" step="1048576" value="16777216"></label><label class="control-field">Total bytes <input id="archive-total-bytes" type="number" min="1" step="1048576" value="134217728"></label></div>
    <div class="deep-status-grid" aria-live="polite"><span>Entries<b id="deep-entries">—</b></span><span>Bytes<b id="deep-bytes">—</b></span><span>Elapsed<b id="deep-time">—</b></span><span>Estimated cost<b id="deep-cost">—</b></span><span>Media<b id="deep-media">—</b></span></div>
  </section>
  <main class="workspace" aria-label="Jikji index browser">
    <nav class="pane explorer-pane" aria-labelledby="explorer-title">
      ...
      <div class="controls selection-controls"><button class="compact" id="index-basic" type="button">Add selected to basic index</button><button class="compact" id="index-content" type="button">Add selected to content index</button></div>
      <div class="controls selection-controls"><label for="view-filter">View</label><select id="view-filter"><option value="all">All files</option><option value="unindexed">Unindexed only</option><option value="basic">Basic index</option><option value="content">Content index</option></select><label for="view-sort">Sort</label><select id="view-sort"><option value="name">Name</option><option value="size">Size</option><option value="mtime">Modified</option><option value="status">Index status</option></select></div>
    ...
    <section class="pane" aria-labelledby="results-title">
      <div class="controls result-controls"><label for="result-sort">Sort results</label><select id="result-sort"><option value="relevance">Relevance</option><option value="name">Name</option><option value="path">Path</option><option value="scope">Index scope</option></select></div>
  ...
  <footer class="pane-head" aria-label="Index management">
    <span class="eyebrow">Folder controls</span><span class="eyebrow" id="job-status">Idle</span><span class="spacer"></span><button id="cancel-job" class="compact" type="button" disabled>Cancel job</button>
  ...
  (() => {
    ...
    const state = { root: "", folder: "", selected: "", selectedPaths: new Set(), query: "", roots: [], confirmAction: null, request: {}, currentJob: "" };
    ...
    async function api(path, values={}, options={}) {
      ...
      try {
        ...
        if (!response.ok) { const detail=payload && (payload.error_detail || payload.error); const message=typeof detail === "string" ? detail : detail?.message || `Request failed (${response.status})`; throw Object.assign(new Error(message), { detail: payload?.error_detail, status: response.status }); }
        return payload;
    ...
    async function loadFiles(folder="") { state.folder=folder;setText("tree-path",folder||"/");setText("folder-context",`Current folder: ${folder||"/"}`);const list=$("file-list");listState(list,"Loading folder","Reading local files…");try{const data=await api("/api/files",{path:folder});let entries=Array.isArray(data.entries)?data.entries:[];const filter=$("view-filter")?.value||"all";if(filter!=="all")entries=entries.filter(entry=>filter==="unindexed"?entry.scope==="unindexed":filter==="content"?entry.scope==="content":entry.scope==="basic");const sort=$("view-sort")?.value||"name";entries.sort((a,b)=>sort==="size"?(Number(b.size)||0)-(Number(a.size)||0):sort==="mtime"?(Number(b.mtime)||0)-(Number(a.mtime)||0):sort==="status"?String(a.scope||a.status).localeCompare(String(b.scope||b.status)):String(a.name||a.path).localeCompare(String(b.name||b.path)));list.replaceChildren();if(folder){const li=document.createElement("li"),up=document.createElement("button");up.className="file-row";up.type="button";up.append(document.createTextNode("↰"),document.createTextNode("Parent folder"));up.addEventListener("click",()=>loadFiles(folder.split("/").slice(0,-1).join("/")));li.append(up);list.append(li);}entries.forEach(entry=>{const li=document.createElement("li"),button=document.createElement("button"),icon=document.createElement("span"),name=document.createElement("span"),size=document.createElement("small");button.type="button";button.className="file-row";button.dataset.path=entry.path;icon.textContent=entry.type==="directory"?"▸":"·";name.textContent=entry.name||entry.path;size.textContent=entry.type==="directory"?(entry.status||"directory"):bytes(entry.size);button.append(icon,name,size);button.addEventListener("click",()=>entry.type==="directory"?loadFiles(entry.path):loadPreview(entry.path));li.append(button);list.append(li);});addSelectionControls();if(!entries.length)listState(list,folder?"Folder is empty":"Explorer is empty",folder?"No local entries in this folder.":"Select a root to browse the local filesystem.");}catch(error){listState(list,error.message.includes("timed out")?"Folder request timed out":"Could not load files",`${errorMessage(error)} Retry with Refresh.`);showError(error);} }
    ...
    async function waitForJob(jobId) { state.currentJob=jobId; $("cancel-job").disabled=false; for (let attempt=0; attempt<120; attempt++) { const job=await api(`/api/jobs/${encodeURIComponent(jobId)}`); setText("job-status",`${job.state} · ${job.progress}%`); if (job.state === "completed") { state.currentJob=""; $("cancel-job").disabled=true; return job.result || job; } if (job.state === "failed") throw new Error(job.error?.message || "Index job failed"); if (job.state === "cancelled") throw new Error("Index job cancelled."); await new Promise(resolve=>setTimeout(resolve,1000)); } throw new Error("Index job status timed out. Retry the action."); }
    async function mutation(path, values, label) { clearError(); const button=$(label); button.disabled=true;button.classList.add("busy");try{let data=await api(path,{...values,token:token()},{method:"POST"});if(data.job_id)data=await waitForJob(data.job_id);if(data.prepared!==undefined)updateStats(data);await Promise.all([loadRoots(),loadStatus(),loadFiles(state.folder),loadIndexed(indexedFolderFor(state.selected))]);toast(`${button.textContent.trim()} complete.`);}catch(error){setText("health","Error");showError(error);}finally{button.disabled=false;button.classList.remove("busy");} }
